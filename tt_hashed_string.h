...
namespace {
    ...
    static constexpr unsigned int _crc_table[256] = {
    ...
    template<size_t idx>
    constexpr uint32_t _crc32(const char* str)
    {
        return (_crc32<idx - 1>(str) >> 8) ^ _crc_table[(_crc32<idx - 1>(str) ^ str[idx]) & 0x000000FF];
    }

    // This is the stop-recursion function
    template<>
    constexpr unsigned int _crc32<size_t(-1)>(const char* str)
    {
        return 0xFFFFFFFF;
    }
}
